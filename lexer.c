...
token_t lexer_get_next_token(lexer_t *lexer)
{
    while(lexer->c != '\0')
    {
        ...
        if (lexer->c == DOUBLE_QUOTES)
            return lexer_get_string(lexer, DOUBLE_QUOTES, TOKEN_STRING_DOUBLE_QUOTES);
        if (lexer->c == SINGLE_QUOTES)
            return lexer_get_string(lexer, SINGLE_QUOTES, TOKEN_STRING_SINGLE_QUOTES);
        if (lexer->c == LEFT_PARENTHESES)
            return lexer_get_string(lexer, RIGHT_PARENTHESES, TOKEN_PARENTHESES);
